...
- **Rediseño estético y espaciado respirable del inventario (COMPLETO 2026-07-28)**: La tabla de inventario en escritorio y la vista de tarjetas en móvil recibieron un rediseño completo de espaciado (padding incrementado a `py-2.5 px-3`, tipografía `text-xs` de 12px), encabezados en Slate/Zinc pulido (`bg-slate-900 text-slate-100`), bordes suavizados (`border-slate-200`) y badges de estado en tonos HSL suaves (*Emerald, Amber, Rose, Indigo*). La funcionalidad se conservó al 100%.
