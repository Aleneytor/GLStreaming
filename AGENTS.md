...
- **Panel del revendedor rediseñado visualmente (COMPLETO 2026-07-28)**: `/dashboard` para rol `revendedor` cuenta con cabecera de bienvenida personalizada (*¡Hola, Name! 👋*), 4 tarjetas KPI (*Activas, Al día, Por vencer, Vencidas*), buscador con icono 🔍 y botón de limpieza, scroll horizontal de chips de plataforma para móviles, tarjetas de cliente con badge de modalidad (*Perfil extra*, *Familiar*, *Cuenta completa*), franja de estado por vencimiento (el borde-color refleja el vencimiento, no la plataforma), pie con `BotonAcceso` y **simplificación de Spotify**: se presenta limpiamente como `Spotify Premium` omitiendo modalidades internas (*Familiar/Individual*).
...
- Cada plataforma se identifica por su **logo de marca** (Simple Icons, en su
  color, solo en el trazo) sobre una tarjeta neutra; las que no tienen logo usan
  una insignia de letra. Ya no hay un color de fondo por `slug` (se quitó en el
  rediseño del 2026-07-29). Cada tarjeta muestra estado, métricas de ocupación y
  acceso táctil a su inventario.
