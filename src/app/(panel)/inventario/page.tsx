...
export default async function InventarioPage() {
  ...
  const [{ data: plataformas }, { data: cuentas }] = await Promise.all([
    supabase.from("plataformas").select("id, nombre, slug").eq("activa", true).order("nombre"),
    supabase
      .from("cuentas")
      .select("id, productos_plataforma!inner ( plataforma_id ), unidades_inventario ( id )")
      .neq("estado", "archivada"),
  ]);

  // Conteo por plataforma (pocos registros: se agrupa en memoria).
  const resumen = new Map<string, { cuentas: number; unidades: number }>();
  for (const c of cuentas ?? []) {
    const rel = c.productos_plataforma as unknown as { plataforma_id: string };
    const previo = resumen.get(rel.plataforma_id) ?? { cuentas: 0, unidades: 0 };
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <ul className="space-y-2">
        {plataformas?.map((p) => {
          const r = resumen.get(p.id) ?? { cuentas: 0, unidades: 0 };
              ...
              >
                <span className="min-w-0">
                  ...
                  >
                    {vacia
                      ? "Sin cuentas todavía"
                      : `${r.cuentas} ${r.cuentas === 1 ? "cuenta" : "cuentas"} · ${r.unidades} ${
                          r.unidades === 1 ? "perfil" : "perfiles"
                        }`}
                  </span>
                </span>
