-- ============================================================================
-- 0025 — Importar también ventas de CUENTA COMPLETA (no solo por perfil)
-- ----------------------------------------------------------------------------
-- HBO, Disney+ y Prime (y Netflix) se venden de dos formas: por perfil o la
-- cuenta entera a un solo cliente (en el Excel, una fila con perfil «Cuenta
-- Completa»). Hasta ahora la importación solo sabía asignar perfiles sueltos.
--
-- Ahora la función mira la MODALIDAD elegida: si su alcance es `cuenta`, crea
-- una asignación de cuenta completa (consume toda la capacidad, sin perfil
-- concreto) en vez de ocupar un solo slot. El resto —cuenta madre, costo del
-- proveedor, período de carga inicial y cobro— es igual.
-- ============================================================================
...
-- Helper: reutiliza el cliente por nombre o lo crea; completa el teléfono si
-- faltaba. Se extrae para no duplicarlo entre las dos ramas de arriba.
...
begin
  ...
  where lower(nombre) = lower(p_nombre) and archived_at is null
  limit 1;
  ...
  elsif nullif(btrim(coalesce(p_whatsapp, '')), '') is not null then
    update public.clientes
    set whatsapp_original = coalesce(whatsapp_original, btrim(p_whatsapp)),
        whatsapp_normalizado = coalesce(
          whatsapp_normalizado,
          nullif(regexp_replace(p_whatsapp, '[^0-9+]', '', 'g'), ''))
    where id = v_id;
  end if;
...
comment on function public.importar_servicio_existente is
  ...
  'perfil (alcance unidad) o vende la cuenta completa (alcance cuenta). Trae '
  'cuenta madre, costo y renovación del proveedor, cliente, período y cobro.';
