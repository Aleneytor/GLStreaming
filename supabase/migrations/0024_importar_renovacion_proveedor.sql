-- ============================================================================
-- 0024 — Importar también la fecha de renovación del proveedor
-- ----------------------------------------------------------------------------
-- El Excel trae, por cuenta, la columna «Renovar»: cuándo toca pagarle al
-- proveedor. Antes esa fecha se derivaba (inicio del cliente + 1 mes), pero el
-- calendario del proveedor es INDEPENDIENTE del calendario del cliente. Ahora
-- se puede fijar explícitamente.
--
-- Se pasa el INICIO del ciclo de proveedor (`p_prov_inicio`, = renovar − 1 mes,
-- calculado en la app). `registrar_ciclo_proveedor` deriva de ahí la próxima
-- renovación (inicio + 1 mes = la fecha del Excel) y el día ancla. Si no viene,
-- se cae al comportamiento anterior (inicio del cliente).
-- ============================================================================
...
drop function if exists public.importar_servicio_existente(
  ...
  date, date, numeric, uuid, numeric, text);
...
begin
  ...
  if v_cuenta_id is null then
    ...
    if p_costo_usdt is not null and p_costo_usdt > 0 then
      -- El inicio del ciclo de proveedor (si se conoce la renovación, viene
      -- calculado como renovar − 1 mes) fija la próxima renovación y el ancla.
...
comment on function public.importar_servicio_existente is
  ...
  'del correo, con su costo y su renovación de proveedor la primera vez), '
  'perfil, cliente, período de carga inicial y cobro opcional.';
