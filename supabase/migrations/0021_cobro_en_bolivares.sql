-- ============================================================================
-- 0021 — El cobro nace en bolívares, y renovar es cobrar
-- ----------------------------------------------------------------------------
-- CORRECCIÓN DE DOMINIO pedida por el dueño del negocio (23/07/2026), que
-- invalida dos supuestos de la Fase 4:
--
--  1. Se creía que el precio se pactaba en USD y los Bs se derivaban a BCV, de
--     modo que un cobro tenía que igualar EXACTAMENTE `precio × BCV`. En la
--     operación real el monto en bolívares varía de un cliente a otro y de un
--     mes a otro. El hecho fuente es **el dinero que entró en bolívares**; el
--     USD pasa a ser una LECTURA (`monto_ves / BCV`), congelada igual que antes.
--     Se elimina, por tanto, la regla de "el cobro iguala el esperado".
--
--  2. Renovar y cobrar eran dos pasos en dos pantallas distintas (`/vencimientos`
--     y `/cobros`), lo que permitía registrar el mismo ingreso dos veces o dejar
--     renovaciones sin cobro por olvido. Pasan a ser UNA sola operación
--     transaccional: `renovar_y_cobrar`.
--
-- Lo que NO cambia: sigue habiendo un solo cobro por período (no hay abonos
-- parciales), las tasas se congelan al confirmar y el reverso es una
-- contrapartida que no borra el original.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- registrar_cobro_cliente — ahora el monto en Bs es el dato de entrada
-- ----------------------------------------------------------------------------
...
begin
  ...
  if p_monto_ves is null or p_monto_ves <= 0 then
    raise exception 'Indica cuántos bolívares recibiste.';
  end if;
  ...
  -- El USD es ahora una lectura del dinero recibido, no un precio pactado.
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'usd_derivado', v_precio, 'bcv', v_bcv.bs_por_usd,
...
comment on function public.registrar_cobro_cliente is
  'Registra los bolívares realmente recibidos por un período y congela BCV y '
  'paralela. El precio en USD se DERIVA del monto (monto_ves / BCV): el hecho '
  'fuente es el dinero que entró, no una tarifa.';

-- ----------------------------------------------------------------------------
-- renovar_y_cobrar — una sola operación, como en la hoja de cálculo
-- ----------------------------------------------------------------------------
...
begin
  ...
  if p_monto_ves is not null and p_monto_ves > 0 then
    perform public.registrar_cobro_cliente(v_periodo_id, p_monto_ves, p_referencia);
  end if;
...
comment on function public.renovar_y_cobrar is
  'Renueva y cobra en la misma transacción: o quedan las dos cosas, o ninguna. '
  'Evita el ingreso registrado dos veces por dos pantallas distintas.';

-- ----------------------------------------------------------------------------
-- vender_unidad — la venta también cobra en el mismo paso
-- ----------------------------------------------------------------------------
-- Se conserva el orden de parámetros y el nuevo se añade AL FINAL, para que las
-- llamadas y suites existentes sigan valiendo. `p_precio_usd` se mantiene por
-- compatibilidad, pero si llega `p_monto_ves` manda el bolívar.
drop function if exists public.vender_unidad(
  uuid, uuid, uuid, uuid, numeric, date, integer, uuid, date, text, text, text);
...
begin
  ...
  if p_monto_ves is not null and p_monto_ves > 0 then
    perform public.registrar_cobro_cliente(v_periodo_id, p_monto_ves, null,
                                           coalesce(p_fecha_venta, p_inicio)::timestamptz);
  end if;
...
comment on function public.vender_unidad is
  ...
  'suscripción + asignación + período. Si se indica `p_monto_ves`, cobra en la '
  'misma transacción. Bloquea la cuenta y aplica la exclusión perfil ↔ cuenta.';

-- ============================================================================
-- Permisos
-- ============================================================================
revoke execute on function public.renovar_y_cobrar(uuid, date, integer, numeric, boolean, text) from public;
grant  execute on function public.renovar_y_cobrar(uuid, date, integer, numeric, boolean, text) to authenticated;
revoke execute on function public.vender_unidad(
  uuid, uuid, uuid, uuid, numeric, date, integer, uuid, date, text, text, text, numeric) from public;
grant  execute on function public.vender_unidad(
  uuid, uuid, uuid, uuid, numeric, date, integer, uuid, date, text, text, text, numeric) to authenticated;
