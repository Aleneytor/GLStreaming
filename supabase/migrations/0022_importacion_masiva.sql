-- ============================================================================
-- 0022 — Importación masiva de la cartera existente (migración desde el Excel)
-- ----------------------------------------------------------------------------
-- Cargar ~115 perfiles extra y ~25 cuentas completas de una en una no es
-- viable. Esta función importa UNA FILA del Excel de forma atómica: o queda
-- todo (cuenta + credenciales + perfil + cliente + período + cobro), o nada.
--
-- POR QUÉ `carga_inicial` Y NO `venta_nueva`: lo que se importa son servicios
-- que YA estaban corriendo antes de existir la app. El dominio ya preveía esto
-- (`sesiones_carga_inicial`, `tipo_operacion = 'carga_inicial'`), y distinguirlo
-- importa: una carga inicial puede tener un período histórico vigente y NO debe
-- contarse como una venta del día en Caja. Por eso `fecha_venta` queda nula.
--
-- FECHAS: aquí se recibe la fecha de renovación EXPLÍCITA, no se calcula. En el
-- Excel el dato que existe es «cuándo vence», y forzarlo a pasar por el cálculo
-- de mes calendario lo deformaría en los casos de fin de mes (un vencimiento el
-- 31 volvería como 28).
--
-- REUTILIZACIÓN DE CUENTA: varias filas pueden compartir la misma cuenta madre
-- (las 5 líneas de una cuenta completa). Se reconocen por `login_fingerprint`,
-- que es el HMAC del correo calculado en la aplicación — nunca viaja el correo
-- en claro.
-- ============================================================================
...
begin
  ...
  if v_nombre_cliente is null then
    return jsonb_build_object(
      ...
      'cuenta_creada', v_creada_cuenta, 'vendida', false);
  end if;
  ...
  return jsonb_build_object(
    'cuenta_id', v_cuenta_id, 'unidad_id', v_unidad_id,
    'cuenta_creada', v_creada_cuenta, 'vendida', true,
...
comment on function public.importar_servicio_existente is
  ...
  'del correo), perfil, cliente, período de carga inicial y cobro opcional. '
  'Exige una sesión de carga abierta.';

-- ----------------------------------------------------------------------------
-- abrir_sesion_carga / cerrar_sesion_carga
-- ----------------------------------------------------------------------------
create or replace function public.abrir_sesion_carga(
...
begin
  ...
  -- Reutiliza la sesión abierta del mismo producto en vez de acumular sesiones.
  select id into v_id
  from public.sesiones_carga_inicial
  where estado = 'abierta'
    and producto_plataforma_id is not distinct from p_producto_id
  ...
  insert into public.sesiones_carga_inicial (
    producto_plataforma_id, fecha_corte, estado, abierta_por_id, motivo
  ) values (p_producto_id, current_date, 'abierta', auth.uid(), p_motivo)
...
create or replace function public.cerrar_sesion_carga(p_sesion_id uuid, p_motivo text default null)
returns void
language plpgsql
set search_path = ''
as $$
begin
  ...
  update public.sesiones_carga_inicial
  set estado = 'cerrada', cerrada_at = now(), cerrada_por_id = auth.uid(),
      motivo = coalesce(p_motivo, motivo)
  where id = p_sesion_id and estado = 'abierta';
end;
$$;

comment on function public.abrir_sesion_carga is
  'Ventana autorizada para cargar cartera previa. Cerrarla deshabilita nuevas '
  'cargas; corregir después exige otra sesión versionada.';
...
revoke execute on function public.importar_servicio_existente(
  ...
  date, date, numeric, uuid) from public;
grant execute on function public.importar_servicio_existente(
  uuid, uuid, integer, text, text, text, text, integer, text, text, uuid, text, text,
  date, date, numeric, uuid) to authenticated;
revoke execute on function public.abrir_sesion_carga(uuid, text) from public;
grant  execute on function public.abrir_sesion_carga(uuid, text) to authenticated;
revoke execute on function public.cerrar_sesion_carga(uuid, text) from public;
grant  execute on function public.cerrar_sesion_carga(uuid, text) to authenticated;
