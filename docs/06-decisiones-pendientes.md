...
| DEC-98 | **El secreto expuesto de Kuanto NO se rota** (decisión del usuario, 23/07/2026): el proyecto funciona y no quiere intervenirlo. GL Streaming procede con la Fase 4 igualmente. | La integración solo **lee** `p2p_rate_history` con la clave pública, distinta de la expuesta (`service_role`, de escritura). El riesgo que permanece es que un tercero inyecte una tasa falsa que GL Streaming congelaría en operaciones reales. Se mitiga **en el lado de GL Streaming**: validación de rango contra la última tasa conocida, control de antigüedad, guardado idempotente por `fuente_registro_id` y prohibición de inventar valores si la fuente falla. `SEC-03` y `RATE-08` quedan cerradas como "asumidas por el usuario". **Ampliación 23/07/2026:** el usuario pide conectar la fuente real y asume expresamente cualquier fallo. La integración queda EN VIVO leyendo `p2p_rate_history` con la clave **anon** de Kuanto (la que su propio frontend entrega a cualquier navegador), nunca con la clave de escritura expuesta. |
| DEC-99 | **Resuelve `CLOSE-02` (23/07/2026):** un dato que llega después de cerrar el mes exige **reapertura versionada**, nunca una edición silenciosa. `reabrir_mes()` marca el cierre anterior como `reemplazado`, crea la versión siguiente y **exige un motivo** que queda en la auditoría junto con el actor. | El cierre anterior no se borra ni se edita: siempre se puede auditar qué se publicó y por qué cambió. `calcular_cierre_mensual()` se niega a recalcular un mes `cerrado` mientras no se reabra. |
| DEC-100 | **Frescura de tasas (23/07/2026):** una tasa solo puede congelarse en una operación si se confirmó en las últimas **24 h**. Como el guardado es idempotente, se añade `tasas_cambio.revalidada_at`, que registra que se comprobó que la observación seguía siendo la última aunque la fuente no publicara nada nuevo. | Sin esta columna, la BCV publicada el viernes bloquearía un cobro perfectamente válido el lunes. La antigüedad que decide es `coalesce(revalidada_at, obtenida_at)`, tanto en SQL (`tasa_utilizable`) como en la app (`confirmadaAt`). |
| DEC-101 | **Una tasa `simulada` no puede congelar una operación (23/07/2026).** El valor simulado existe para desarrollar sin la fuente externa, pero `tasa_utilizable` lo descarta y tampoco sirve de referencia para el control de desviación. | Sin esto, un cobro o un gasto podía quedar grabado para siempre con un valor inventado, justo lo contrario de la regla «nunca se inventa un valor». Además, al conectar la fuente real el salto 40 → 855 habría sido rechazado por el guardia del 50 %. Migración `0020`. |
...
| ~~CLOSE-02~~ | **Resuelto por `DEC-99`:** reapertura versionada con motivo y actor auditados. | Ver `DEC-99`. |
